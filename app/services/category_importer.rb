...
class CategoryImporter
  ...
  def validate_file
    ...
    if errors.count > MAX_ALLOWED_ERRORS
      return failure([ "File has too many errors. The format is either invalid, missing too many fields, or you are missing Categories. Always ims and Categories first." ])
    end
  ...
  def normalize_row(row)
    {
      "min_age"    => row["min_age"]&.strip&.titleize,
      "max_age"    => row["max_age"]&.strip&.titleize,
      "sex"        => row["sex"]&.strip,
